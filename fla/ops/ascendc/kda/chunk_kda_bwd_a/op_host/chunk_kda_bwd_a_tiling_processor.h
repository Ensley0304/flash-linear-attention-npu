...
#include "../op_kernel/chunk_kda_bwd_a_struct.h"
...
public:
    ...
    ge::graphStatus Process()
    {
        ...
        OP_CHECK_IF(aqk.GetDim(dimAxis) != 64 || tiling_.keyDim != 128,
                    ...
                    return ge::GRAPH_FAILED);
        OP_CHECK_IF(tiling_.valueDim != 128 && tiling_.valueDim != 256,
                    OP_LOGE(ctx_.nodeName, "Kernel A requires V=128 or V=256."),
                    return ge::GRAPH_FAILED);
        OP_CHECK_IF(vNew.GetDim(dimAxis) != static_cast<size_t>(tiling_.valueDim) ||
                        dO.GetDim(dimAxis) != static_cast<size_t>(tiling_.valueDim),
        ...
        if (!hasCu) {
            ...
            OP_CHECK_IF(h.GetDim(0) != static_cast<size_t>(tiling_.batch) ||
                            h.GetDim(1) != static_cast<size_t>(tiling_.chunkNumPerBatch) ||
                            h.GetDim(2) != static_cast<size_t>(tiling_.headNum) ||
                            h.GetDim(3) != 128 ||
                            h.GetDim(4) != static_cast<size_t>(tiling_.valueDim),
                        OP_LOGE(ctx_.nodeName,
            ...
            OP_CHECK_IF(h.GetDim(0) != static_cast<size_t>(tiling_.chunkNum) ||
                            ...
                            h.GetDim(2) != 128 ||
                            h.GetDim(3) != static_cast<size_t>(tiling_.valueDim),
                        OP_LOGE(ctx_.nodeName,
