...
Finalize 在 Ascend950 上禁用编译器跳转表（`-fno-jump-tables`）。
CANN 9.1.0 / msSanitizer 26.1.0 的动态插桩复现中，跳转表访问后出现
507015，并伴随启动参数区域的非法读取报告；禁用跳转表后同一输入可完成检测。
此选项仅改变分支代码生成，不修改数学运算、任务划分、workspace 或同步协议。
更新编译器或检测工具后，须重新验证该兼容性选项是否仍有必要。
