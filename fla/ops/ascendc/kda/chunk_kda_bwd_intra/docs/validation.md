...
> 2026-07-22 stage-1 event redesign: changing physical cards did not alter the grouped timeout, and handshake plus both stage-0 GEMMs already returned. The next artifact therefore keeps grouped key 23 active but gives both MMAD layouts one persistent, serialized local-event owner and disjoint C workspace. Acceptance order is strict: smallest grouped single launch, directed precision guards, complete 37-case suite, then same-card msprof. Until all four gates pass, key 23 remains an optimization candidate rather than a delivery claim.
