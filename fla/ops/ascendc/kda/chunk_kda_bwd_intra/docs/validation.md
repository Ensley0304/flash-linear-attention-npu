...
| 项目 | 状态 | 说明 |
|---|---|---|
| Python 语法 | 已通过 | NPU pytest 已通过 `py_compile`；当前服务器测试文件应收集 37 项 |
...
| Cube 源码隔离契约 | 本地待复核 | key19 为独立 MIX key；public 只调用一次 stage-5 L0；PR190 四槽/HV-pair/raw-flag 调度与 `chunk_kda_fwd` FP32 BlockMmad 配置均由源码契约固定；一处 host 常量可恢复 key13 |
| patch 卫生 | 已通过 | `git diff --check` 无错误 |
| CANN host/kernel 编译 | key19 待验证 | 必须用单算子 clean build，确认 wheel 中新设备对象哈希 |
| AscendC NPU 精度 | key19 待验证 | 先过 endpoint，再过 repeated-launch 和完整 37 项；不放宽原容差 |
| Profiling/性能优化 | key13 已采集 | key7 48.660 ms、BK64 key12 32.477 ms、key13 31.034 ms；key19 要求每次调用只有一条同时含 AIC/AIV 时间的 MIX KDA 记录 |
...
canary、dense random、Cube repeated-launch、八路 full-Cube path canary 和源码契约，当前服务器
测试文件完整收集数为 37 项；尚未上板前不声明新增路径已通过。
...
## key19 PR190-style MIX 实验门禁

实验 fastpath 仅面向 `safe_gate=true`、BF16、dense、`B=1`、`H=HV`、`BT=64`、`K=128`
和满 chunk。源码契约要求 key19 为独立 `KERNEL_TYPE_MIX_AIC_1_2` 分支，public 只调用一次
stage-5 L0。AIC/AIV 必须使用 PR190 相同的 chunk 分核、HV-pair、四槽双窗口与普通 flag id 2/4
协议；Cube 必须使用仓内 `chunk_kda_fwd` 相同的 IEEE-FP32 BlockMmad policy 和
`64x64x64` tile。key15 和 key16/17/18 继续编译但不得被 public fastpath 选择。

稳定 key13 是目标 shape 的已知可运行 fallback，key7 是通用 fallback。key19 workspace 为
`usedCoreNum * 4 * 600 KiB`，20 个逻辑 AIC 约 46.9 MiB。建议按以下顺序放行：

1. 源码契约和 Python collect 通过；
2. 单算子快速 build，确认 key13 回退和 key19 MIX 实例均进入 wheel；
...
key13 保留为已知可运行的 BK64 回退；key19 必须重新执行第 2～6 项。未满足第 3～6 项前，
不扩大实验 fastpath eligibility，也不移除 key7/key13。
