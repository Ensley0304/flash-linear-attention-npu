...
> 2026-07-22 stage-1 event redesign: changing physical cards did not alter the grouped timeout, and handshake plus both stage-0 GEMMs already returned. The persistent shared-event artifact also timed out, so the next artifact keeps grouped key 23 and its disjoint C workspace but restores one complete event envelope per logical GEMM. Earlier scoped evidence used the old aliased C/B layout and therefore did not test this combination. Acceptance order is strict: smallest grouped single launch, directed precision guards, complete 37-case suite, then same-card msprof. Until all four gates pass, key 23 remains an optimization candidate rather than a delivery claim.
