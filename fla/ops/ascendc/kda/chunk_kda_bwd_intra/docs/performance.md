...
> 2026-07-22 stability gate: the immediate delivery default is the measured 48.660 ms AIV key 7 because every key-23 event variant, including scoped events with disjoint C storage, timed out in the first target-shape launch. A clean key-7-only wheel has now reconfirmed target-shape exit and precision (`1 passed in 10.01s`); the stable-key full37 regression is still pending. The final objective remains below 10 ms for `B=1,T=8192,H=HV=32,K=128,BT=64,BF16,safe_gate=true`; key 15/23 remain compile-only experiments until their synchronization is redesigned from a stable baseline.
