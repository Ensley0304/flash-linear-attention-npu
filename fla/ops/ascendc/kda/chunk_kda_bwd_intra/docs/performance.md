...
> 2026-07-22 stability gate: the immediate delivery default is AIV key 7 because every key-23 event variant, including scoped events with disjoint C storage, timed out in the first target-shape launch. The measured pre-rollback key-7 baseline is 48.660 ms. Although a key-7-only target preflight returned, the later stable directed suite found a NaN in the new pair-bridge extreme-value guard (`14 passed, 1 failed`). The directional-endpoint correction changes only four gate references and keeps the same vector instruction count, so no material performance change is expected, but profiling is blocked until its quick two-case gate and full37 pass. The final objective remains below 10 ms for `B=1,T=8192,H=HV=32,K=128,BT=64,BF16,safe_gate=true`; key 15/23 remain compile-only experiments until repaired from a stable baseline.
