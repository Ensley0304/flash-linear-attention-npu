...
**Safe baseline and current experiment (2026-07-23):** key7 retains the clean AIV block-wise snapshot from commit `75535cd`. That exact snapshot passed all 22 then-existing NPU tests and measured 48.660 ms kernel time (51.107 ms end-to-end) for `B=1,T=8192,H=HV=32,K=128,BT=64,BF16,safe_gate=true`. The single-launch key12 `AIV pack -> FP32 Cube -> AIV consume` fallback uses a 64-feature tile and measures 32.477 ms in the same profiling shape. Key13 keeps key12 intact, batches 16 contiguous source gates into each `Sub/Muls/Exp` repeat, and measures 31.034 ms kernel time. Key14 additionally batches row post-scale operations but measured 31.036 ms, so it is retained only as a no-gain experiment. Key13 is again the active target-shape path. Key15, which moves all four causal contraction families to six block-diagonal FP32 Cube GEMMs, remains compiled but is quarantined after the endpoint guard timed out; host tiling no longer selects it. Every non-eligible shape still dispatches to key7 or the existing legacy keys. Key15 must be rebuilt around a proven Cube completion protocol and pass endpoint, repeated-launch, full precision, and profiling gates before it can be re-enabled.

The primary delivery path is `safe_gate=true`; `safe_gate=false` remains a separate compiled branch for compatibility. The stable safe fallback uses one AIV launch, 16-token block-wise accumulation, resident `[chunk, 32]` q/k/g feature tiles, FP32 accumulation, dense/GVA/varlen support, and no sequence-sized workspace. The narrowly eligible MIX paths use one launch. The active key13 path uses its proven task-sized workspace formula; the quarantined key15 experiment uses a bounded 600 KiB workspace slot per logical AIC core (about 12 MiB on 20 cores). Key7 remains the general fallback.
