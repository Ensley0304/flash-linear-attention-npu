...
**Validation status (2026-07-22):** grouped key 23 remains the active `<10 ms` Cube candidate, but is not yet a delivery or profiling baseline. Earlier variants timed out at stage 1 on physical devices 2 and 3. The current candidate gives its two layout-specific MMAD engines one serialized local-event owner, keeps their L1/L0 storage disjoint, and writes all Fixpipe results to a dedicated workspace tail instead of overwriting B inputs. It must pass a clean-wheel single-launch exit proof before precision and performance claims are accepted.
...
The source defaults `KDA_GROUPED_ENABLE_UNIT_FLAG=false` and `KDA_GROUPED_PERSISTENT_MMAD_ENGINES=true`. The row-major and column-major engines retain disjoint L1/L0 partitions but intentionally share event IDs 0--3 and L0C event 0. Only the left engine initializes and drains that event domain; every one of the 17 serialized GEMMs consumes and returns the same free tokens. A dedicated 22,528-element C tail expands each workspace slot from 26,624 to 49,152 FP32 elements and removes every GM input/output alias. This protocol targets the stage-1 timeout without changing GEMM order, gate formulas, or FP32 accumulation order, but still requires clean-wheel A2 exit and precision evidence. The source additionally defaults `KDA_GROUPED_REUSE_VECTOR_MASK=true`: every aligned FP32 arithmetic envelope programs one 64-lane normal mask, uses the matching low-level `isSetMask=false` overloads without changing instruction or repeat order, and resets the mask before leaving the envelope. Setting it to `false` restores the per-call Level-2 API path for clean-wheel precision/performance bisection. A2 is the current validation target; A3 requires an independent device run. Pair-wise key 15 remains compiled as a rollback; unsupported shapes and `safe_gate=false` continue to select keys 0--7 at runtime.
