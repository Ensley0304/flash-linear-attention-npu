...
**Safe baseline and current experiment (2026-07-22):** key7 retains the clean AIV block-wise snapshot from commit `75535cd`. That exact snapshot passed all 22 then-existing NPU tests and measured 48.660 ms kernel time (51.107 ms end-to-end) for `B=1,T=8192,H=HV=32,K=128,BT=64,BF16,safe_gate=true`. The deadlocking key-15/key-23 mixed-Cube implementation and its cross-core protocol remain removed. A new narrowly eligible experiment moves only rowBlock3's off-left contraction into three same-stream stages (`AIV prep -> FP32 Cube -> AIV consume`) with no CrossCore synchronization; every non-eligible shape still dispatches to key7 or the existing legacy keys. The new path is not considered validated until the documented 28-test NPU gate and total-stage profiling pass.
