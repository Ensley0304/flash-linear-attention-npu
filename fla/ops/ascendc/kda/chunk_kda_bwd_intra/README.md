...
**Validation status (2026-07-22):** the scoped-event, non-aliasing grouped key 23 artifact also timed out in the first BF16 target-shape launch. Delivery dispatch is therefore rolled back to the last device-proven block-wise AIV key 7; both mixed key 15 and grouped key 23 remain compiled but are unreachable from normal tiling. A clean key-7-only wheel reconfirmed the formerly hanging BF16 target shape on physical device 3 (`1 passed in 10.01s`). The next gate is a one-build stable-key (`0,2,5,7`) full37 regression before `<10 ms` Cube work resumes.
