...
**Validation status (2026-07-22):** grouped key 23 remains the active `<10 ms` Cube candidate, but is not yet a delivery or profiling baseline. Earlier variants timed out at stage 1 on physical devices 2 and 3. The current candidate keeps all 17 Cube GEMMs and grouped AIV caching, writes every Fixpipe result to a dedicated workspace tail, and restores one complete non-UnitFlag event envelope per logical GEMM. This exact combination of non-aliasing C storage and scoped events has not previously been tested on device. It must pass a clean-wheel single-launch exit proof before precision and performance claims are accepted.
...
The primary implementation mode is `safe_gate=true`; `safe_gate=false` remains a separate compiled branch for compatibility. General safe shapes use the 16-token AIV block-wise implementation. The exact DAV_2201 target domain (`BF16`, `BT=64`, `K=128`, dense full chunks, `H=HV`) selects grouped key 23, the experimental four-stage, 17-GEMM AIC/AIV design described below.
...
The source defaults `KDA_GROUPED_ENABLE_UNIT_FLAG=false` and `KDA_GROUPED_PERSISTENT_MMAD_ENGINES=false`. Each logical GEMM owns a complete `preSetFlags -> operator() -> finalWaitFlags` transaction, matching the repository-proven key-15 lifecycle. A dedicated 22,528-element C tail expands each workspace slot from 26,624 to 49,152 FP32 elements and removes every GM input/output alias. Only the local event lifetime is rolled back: key 23 dispatch, all 17 Cube GEMMs, grouped AIV feature/gate caching, double-slot GM bridge, formulas, and FP32 accumulation order remain unchanged. Persistent engines stay compiled for a later bounded-batch experiment after the scoped path has device evidence. The source additionally defaults `KDA_GROUPED_REUSE_VECTOR_MASK=true`: every aligned FP32 arithmetic envelope programs one 64-lane normal mask, uses the matching low-level `isSetMask=false` overloads without changing instruction or repeat order, and resets the mask before leaving the envelope. Setting it to `false` restores the per-call Level-2 API path for clean-wheel precision/performance bisection. A2 is the current validation target; A3 requires an independent device run. Pair-wise key 15 remains compiled as a rollback; unsupported shapes and `safe_gate=false` continue to select keys 0--7 at runtime.
