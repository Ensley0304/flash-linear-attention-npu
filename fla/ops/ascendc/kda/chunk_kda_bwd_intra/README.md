...
The source defaults `KDA_GROUPED_PERSISTENT_MMAD_ENGINES=false`. Each of the 17 logical GEMMs owns one complete `preSetFlags -> operator() -> finalWaitFlags` transaction, matching the repository's validated `MmadPingpongTlaMulti` contract. The earlier multi-invocation UnitFlag/Fixpipe envelope is the high-confidence deadlock cause: it first appears in grouped stage 1 and exactly matches the observed stage-0-pass/full-path-timeout boundary. The persistent implementation therefore remains compile-time-only and must not be used for delivery until a new protocol has independent device evidence. This rollback does not change GEMM order, workspace layout, or output visibility through `PIPE_FIX`. The source additionally defaults `KDA_GROUPED_REUSE_VECTOR_MASK=true`: every aligned FP32 arithmetic envelope programs one 64-lane normal mask, uses the matching low-level `isSetMask=false` overloads without changing instruction or repeat order, and resets the mask before leaving the envelope. Setting it to `false` restores the per-call Level-2 API path for clean-wheel precision/performance bisection. A2 is the current validation target; A3 requires an independent device run. Key 23 and all scheduling candidates are under clean-build/NPU validation. Pair-wise key 15 remains compiled as a source-level rollback that requires disabling grouped dispatch and rebuilding; unsupported shapes and `safe_gate=false` continue to select keys 0--7 at runtime.
