...
**Validation status (2026-07-22):** the scoped-event, non-aliasing grouped key 23 artifact also timed out in the first BF16 target-shape launch. Delivery dispatch is therefore rolled back to the last device-proven block-wise AIV key 7; both mixed key 15 and grouped key 23 remain compiled but are unreachable from normal tiling. Key 7 previously passed the clean-wheel 22-case suite and repeated launches. The `<10 ms` Cube work resumes only after this stable path is reconfirmed.
