...
**Safe baseline and current experiment (2026-07-23):** key7 retains the clean AIV block-wise snapshot from commit `75535cd`. That exact snapshot passed all 22 then-existing NPU tests and measured 48.660 ms kernel time (51.107 ms end-to-end) for `B=1,T=8192,H=HV=32,K=128,BT=64,BF16,safe_gate=true`. The single-launch key12 `AIV pack -> FP32 Cube -> AIV consume` fallback uses a 64-feature tile and measures 32.477 ms in the same profiling shape. Key13 keeps key12 intact, batches 16 contiguous source gates into each `Sub/Muls/Exp` repeat, and measures 31.034 ms kernel time. Key14 additionally batches row post-scale operations but measured 31.036 ms, so it is retained only as a no-gain experiment. Key15, which moves all four causal contraction families to six block-diagonal FP32 Cube GEMMs, remains compiled but is quarantined after the endpoint guard timed out. The new keys 16/17/18 are an intentionally conservative split-launch experiment: AIV packs only the previous-left and diagonal-left matrices, AIC evaluates them as independent `64x64xK` IEEE-FP32 TileMmad calls with explicit PR190-style L1/L0 event closure, and a final AIV launch consumes C while retaining the proven right-half vector path. K remains whole in each tile so the FP32 dot-product reduction is not reassociated. They use no MIX task type and no cross-core flags. One host constant restores the proven key13 stage-4 launch; every non-eligible shape still dispatches to key7 or the legacy keys.

The primary delivery path is `safe_gate=true`; `safe_gate=false` remains a separate compiled branch for compatibility. The stable safe fallback uses one AIV launch, 16-token block-wise accumulation, resident `[chunk, 32]` q/k/g feature tiles, FP32 accumulation, dense/GVA/varlen support, and no sequence-sized workspace. Keys 16/17/18 are restricted to dense `B=1,H=HV,BT=64,K=128,BF16` and at most 4096 `(chunk,head)` slots; their compact FP32 A/B/C intermediates are task-sized. The quarantined key15 experiment retains its bounded 600 KiB workspace slot per logical AIC core. Key13 and key7 remain the immediate target-shape and general fallbacks.
