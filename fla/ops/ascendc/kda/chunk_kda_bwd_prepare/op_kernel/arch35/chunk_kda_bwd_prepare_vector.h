...
__simd_vf__ inline void ApplyTriScaleMaskRegbase(
    __ubuf__ float *output, __ubuf__ float *input,
    __ubuf__ float *mask, uint16_t validRows)
{
    constexpr uint32_t ROW = KDA_PREPARE_CHUNK;
    MaskReg full = CreateMask<float, MaskPattern::ALL>();
    ...
    for (uint16_t row = 0; row < validRows; ++row) {
        ...
        LoadAlign(rawInput, input + offset);
...
public:
        ...
        const ChunkKdaBwdPrepareTilingData *tiling, AscendC::TPipe *pipe)
    {
        ...
        // Stream full-size outputs to GM; the following kernels consume them.
        ...
        pipe_->InitBuffer(rawPing_, KDA_PREPARE_RAW_FP32_BYTES);
        pipe_->InitBuffer(rawPong_, KDA_PREPARE_RAW_FP32_BYTES);
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int64_t workTask = blockIdx; workTask < tiling_->workTaskNum;
            ...
            for (int64_t head = headBegin; head < headEnd; ++head, ++generation) {
                ...
                // dAqk: preserve FP32 Cube output through mask/scale and GM.
                ...
                // The raw FP32 UB slot is free after the Vector pipeline has
