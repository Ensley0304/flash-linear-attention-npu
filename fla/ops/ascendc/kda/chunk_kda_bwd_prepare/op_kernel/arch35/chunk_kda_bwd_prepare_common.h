...
constexpr uint32_t KDA_PREPARE_RAW_FP32_BYTES =
...
constexpr uint32_t KDA_PREPARE_A_UB_BYTES =
    2 * KDA_PREPARE_RAW_FP32_BYTES + 3 * KDA_PREPARE_FP32_BYTES;
    ...
    const ChunkKdaBwdPrepareTilingData &tiling, const ChunkInfo &chunk, int64_t head)
{
    ...
    return ((chunk.batchIndex * tiling.denseChunkNum + chunk.stateIndex) * tiling.NV + head) *
           tiling.K * tiling.V;
}
